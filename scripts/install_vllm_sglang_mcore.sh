...
if [ $USE_SGLANG -eq 1 ]; then
    uv pip install "sglang[all]==0.4.6.post1" --no-cache-dir --find-links https://flashinfer.ai/whl/cu124/torch2.6/flashinfer-python && pip install torch-memory-saver --no-cache-dir
fi
uv pip install --no-cache-dir "vllm==0.9.1" torch torchvision torchaudio tensordict torchdata
